...
import kotlinx.android.synthetic.main.activity_prescription_page.*

class PrescriptionPage : AppCompatActivity() {
    ...
    private fun fetchData() {
        ...
        firestore.collection("Users").document(pm).get().addOnSuccessListener {
            if ((it["prescription"].toString() == "").not())
                Glide.with(this).load(it["prescription"]).into(image)
        }
    }
}